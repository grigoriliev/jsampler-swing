...
import static com.grigoriliev.jsampler.JSPrefs.FIRST_MIDI_BANK_NUMBER;
import static com.grigoriliev.jsampler.JSPrefs.FIRST_MIDI_PROGRAM_NUMBER;
...
public class JSMidiInstrumentTree extends JTree {
	...
	removeSelectedInstrumentOrBank() {
		...
		if(obj instanceof InstrTreeNode) {
			...
			if(c > 1) {
				String s;
				s = StdI18n.i18n.getMessage("JSMidiInstrumentTree.removeInstruments?", c);
	...
	private class MidiBank {
		...
		toString() {
			...
			return StdI18n.i18n.getLabel("JSMidiInstrumentTree.MidiBank.name", i + id);
		}
	}
	...
	copyOrMoveMidiBankTo(boolean copy) {
		...
		if(copy) dlg.setTitle(StdI18n.i18n.getLabel("JSMidiInstrumentTree.copyTo"));
		else dlg.setTitle(StdI18n.i18n.getLabel("JSMidiInstrumentTree.moveTo"));
		...
		if(smap == null) {
			SHF.showErrorMessage(StdI18n.i18n.getMessage("JSMidiInstrumentTree.noMap!"), this);
			return;
		}
		...
		if(dlg.getMidiBank() == bank.getId() && smap.getMapId() == getMidiInstrumentMap().getMapId()) {
			...
			SHF.showErrorMessage(StdI18n.i18n.getMessage(s), this);
			return;
		}
	...
	class ContextMenu extends MouseAdapter {
		...
		private final JMenu changeProgramMenu =
			new JMenu(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.changeProgram"));
		
		private final JMenuItem miMoveInstrumentUp =
			new JMenuItem(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.moveUp"));
		
		private final JMenuItem miMoveInstrumentDown =
			new JMenuItem(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.moveDown"));
		...
		ContextMenu() {
			JMenuItem mi = new JMenuItem(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.moveTo"));
			bankMenu.add(mi);
			...
			mi = new JMenuItem(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.copyTo"));
			bankMenu.add(mi);
			mi.addActionListener(new ActionListener() {
				public void
				actionPerformed(ActionEvent e) {
			...
			mi = new JMenuItem(StdI18n.i18n.getMenuLabel("ContextMenu.delete"));
			bankMenu.add(mi);
			mi.addActionListener(new ActionListener() {
				public void
				actionPerformed(ActionEvent e) {
			...
			mi = new JMenuItem(StdI18n.i18n.getMenuLabel("ContextMenu.edit"));
			...
			mi = new JMenuItem(StdI18n.i18n.getMenuLabel("JSMidiInstrumentTree.ContextMenu.moveTo"));
			instrumentMenu.add(mi);
			mi.addActionListener(new ActionListener() {
				public void
				actionPerformed(ActionEvent e) {
		...
		updateProgramGroupMenu(JMenu menu, int prgStart, int prgEnd) {
			...
			menu.setText(StdI18n.i18n.getMenuLabel(s, grp));
...
class JSOverrideInstrumentsConfirmDlg extends OkCancelDialog {
	private final JLabel lMsg = new JLabel(StdI18n.i18n.getMessage("JSOverrideInstrumentsConfirmDlg.lMsg"));
